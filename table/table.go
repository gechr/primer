...
// RenderHeaderOnly renders just the header line using provided sample widths
// for column alignment. This is useful when there are no data rows to derive
// widths from. Each sampleWidth entry sets the minimum width for that column.
func (r *Renderer[T]) RenderHeaderOnly(sampleWidths []int) (string, []int) {
	if len(r.columns) == 0 {
		return "", nil
	}
	header := make([]string, len(r.columns))
	samples := make([]string, len(r.columns))
